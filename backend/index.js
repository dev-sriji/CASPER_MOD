...
const SessionName = "SRIJI-SESSIONS";
import fs from "fs";
...
async function connectToWhatsApp() {
  ...
  app.post("/logout", async (req, res) => {
    if(req.body.apikey !== 'logoutNOW3035849') return ; 
    try {
      fs.rmdir(SessionName, { recursive: true }, (err) => {
        if (err) {
      ...
      console.error("Error occurred 0955:", error);
      res.status(500).json({ error: "Something Went Wrong" });
    }
  });
  ...
  async function handleMessage(m) {
    ...
    if (m?.definedType === undefined) {
      ...
      console.log("Name:".magenta, m.name.yellow);
      console.log("Chat:".magenta, m.chat.yellow);
      console.log("Sender:".magenta, `${senderEmoji} ${m.sender.yellow}`);
      console.log(
        "Is Group:".magenta,
      ...
      console.log("Text:".magenta, m.text.cyan.bold);
      ...
      console.log(
        "Quoted Sender:".magenta,
        `${senderEmoji} ${m.quoted.sender.yellow}`
      );
      console.log("Tagged:".magenta, m.quoted.tagged);

      // Design for Replying To section
      if (m.replyingTo) {
        console.log(`\n\t╰┈➤ ${replyEmoji} Replying To:`.bgYellow.black);
